...
const INVITE_ONLY_MESSAGE = "Sign up is currently unavailable for this email address."

interface SignUpScreenProps {
  emailPasswordAuthEnabled: boolean
}

export function SignUpScreen({ emailPasswordAuthEnabled }: SignUpScreenProps) {
  ...
  const redirectToInviteOnlyError = (message?: string) => {
    router.replace(
      `/auth/error?error=invite_only&error_description=${encodeURIComponent(
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        const message = getAuthErrorMessage(result.error, "Google sign up failed.")
        if (message === INVITE_ONLY_MESSAGE) {
          redirectToInviteOnlyError(message)
          return
        }
        setError(message)
      }
      ...
      const message = getAuthErrorMessage(err, "An unexpected error occurred.")
      if (message === INVITE_ONLY_MESSAGE) {
        redirectToInviteOnlyError(message)
        return
      }
  ...
  const handleEmailSignUp = async (e: FormEvent) => {
    ...
    try {
      ...
      if (signUpError) {
        const message = getAuthErrorMessage(signUpError, "Failed to create account.")
        if (message === INVITE_ONLY_MESSAGE) {
          redirectToInviteOnlyError(message)
          return
        }
    ...
    <AuthScreenShell>
      <div className="mb-8">
        <div className="mb-3 flex items-center gap-2.5">
          <div className="flex size-9 items-center justify-center rounded-md bg-[var(--icon-soft)]">
